...
import { Search, Plus, Tag, Code, Trash2, Copy, Check } from 'lucide-react'
...
const languages = [
  ...
  'html', 'css', 'json', 'sql', 'bash', 'go', 'rust', 'php', 'ruby'
]
...
const getCodePlaceholder = (language: string): string => {
  ...
  return placeholders[language] || `// ${language} example\n// Enter your code here...`
}
    ...
    if (editingSnippet) {
      setSnippets(snippets.map(s => 
        s.id === editingSnippet.id 
          ? { ...s, ...formData, tags: formData.tags.split(',').map(t => t.trim()).filter(Boolean) }
          : s
      ))
    } else {
        ...
        {filteredSnippets().length === 0 ? (
          <div className="text-center py-16">
            ...
            <p className="text-slate-600 dark:text-slate-400 text-lg">
              {snippets.length === 0 ? 'No snippets yet. Add your first one!' : 'No snippets match your search.'}
            </p>
          </div>
        ) : (
